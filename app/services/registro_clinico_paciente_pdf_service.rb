...
class RegistroClinicoPacientePdfService
  ...
  def generate
    Prawn::Document.new(
      ...
      # Conteúdo do registro clínico (sem tratamento)
  ...
  def draw_patient_info(pdf)
    ...
    # Tabela com informações do paciente (sem CPF)
    patient_data = [
      ["Nome:", @paciente.nome],
  ...
  def draw_footer(pdf)
    ...
    pdf.text "Documento destinado ao paciente - Informações de tratamento restritas", 
             size: 8, align: :center, color: '9CA3AF'
  end
